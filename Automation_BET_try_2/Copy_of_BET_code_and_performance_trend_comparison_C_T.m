...
for i = 1:1:length(Velocities_structure)
    ...
    plot(Velocities_structure(i).V,C_T(i,:),'LineWidth',1.5); hold on  
end
legend(num2str(RPMs'),'Location','East')
grid on;hold on;
xlabel('V_{mph}');ylabel('C_T')
title('14x13 propeller C_{T} at different RPMs (BEMT code)')
...
for i = 1:1:length(V_mat(1,:))
    plot(V_mat(:,i),Ct_mat(:,i));hold on    
end
legend(num2str(RPMs'),'Location','East')
grid on;hold on;
xlabel('V_{mph}');ylabel('C_T')
title('14x13 propeller C_{T} at different RPMs (PERF File)')
