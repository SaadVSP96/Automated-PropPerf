...
alpha_deg(1:no_elements,1) = 20;%deg;
alpha_rad(1:no_elements,1) = deg2rad(alpha_deg(1:no_elements,1));%rad

for i = 1:1:no_elements
    Beta_rad(i) = deg2rad(Beta_deg(i));
    for j = 1:1:20
        [pol, ~] = xfoil('ClarkY.dat',alpha_deg(i,j),(10*Re(i)),M(i),'oper iter 200');
        C_l_eqn(i,j)= pol.CL;  C_d_eqn(i,j) = pol.CD;
        
        V_E(i,j) = sqrt(((w(i,j)+V_0_fts)^2)+(Omega_x_r(i)^2));
        
        f_w(i,j) = (((8*pi*r(i))/(n_blades*c(i)))*w(i,j))-((V_E(i,j)/...
        (V_0_fts+w(i,j)))*((C_l_eqn(i,j)*Omega_x_r(i))-(C_d_eqn(i,j)...
        *(w(i,j)*V_0_fts))));
    
        f_prime_w(i,j) = ((8*pi*r(i))/(n_blades*c(i))) - (((C_l_eqn(i,j)...
            *Omega_x_r(i)))*((1/V_E(i,j))-(V_E(i,j)/((V_0_fts+w...
        (i,j))^2)))) + (C_d_eqn(i,j)*((w(i,j)*V_0_fts)/(V_E(i,j))));
    ...
    [poll, ~] = xfoil('ClarkY.dat',alpha_deg(i,end),(10*Re(i)),M(i),'oper iter 1000');
    C_l(i) = poll.CL ; C_d(i) = poll.CD;
